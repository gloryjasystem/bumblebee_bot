...
# ══════════════════════════════════════════════════════════════
# Экспорт базы ЧС — экран выбора типа выгрузки
# ══════════════════════════════════════════════════════════════

@router.callback_query(F.data.startswith("bl_export:"))
async def on_bl_export_menu(callback: CallbackQuery, platform_user: dict | None):
    if not platform_user:
        return
    chat_id = int(callback.data.split(":")[1])
    owner_id = platform_user["user_id"]
    ...
    by_uid  = await db.fetchval(
        "SELECT COUNT(*) FROM blacklist WHERE owner_id=$1 AND user_id IS NOT NULL", owner_id) or 0
    by_name = total - by_uid

    await callback.message.edit_text(
        "<blockquote>Здесь хранятся все заблокированные пользователи вашего бота.\n\n"
        ...
        f"🆔 <b>По Telegram ID:</b> {by_uid:,}\n"
        f"👤 <b>По username:</b> {by_name:,}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            [InlineKeyboardButton(text="📥 Выгрузить всю базу (CSV)", callback_data=f"bl_export_all:{chat_id}")],
            [InlineKeyboardButton(text="🆔 Выгрузить по ID",          callback_data=f"bl_export_uid:{chat_id}")],
            [InlineKeyboardButton(text="👤 Выгрузить по username",    callback_data=f"bl_export_uname:{chat_id}")],
            [InlineKeyboardButton(text="◀️ Назад",                    callback_data=f"ch_protection:{chat_id}")],
        ]),
    )
    await callback.answer()
...
async def _send_bl_csv(callback: CallbackQuery, owner_id: int, chat_id: int,
                       where_extra: str = "") -> None:
    """Генерирует CSV и отправляет файлом."""
    import io, csv
    from aiogram.types import BufferedInputFile

    rows = await db.fetch(
        f"SELECT user_id, username, reason, created_at "
        f"FROM blacklist WHERE owner_id=$1 {where_extra} ORDER BY created_at DESC",
        owner_id,
    )
    ...
    await callback.message.answer_document(
        BufferedInputFile(data, filename="blacklist.csv"),
        caption=f"📤 Экспорт базы ЧС — {len(rows):,} записей",
    )
    await callback.answer()


@router.callback_query(F.data.startswith("bl_export_all:"))
async def on_bl_export_all(callback: CallbackQuery, platform_user: dict | None):
    if not platform_user:
        return
    await _send_bl_csv(callback, platform_user["user_id"],
                       int(callback.data.split(":")[1]))


@router.callback_query(F.data.startswith("bl_export_uid:"))
async def on_bl_export_uid(callback: CallbackQuery, platform_user: dict | None):
    if not platform_user:
        return
    await _send_bl_csv(callback, platform_user["user_id"],
                       int(callback.data.split(":")[1]),
                       where_extra="AND user_id IS NOT NULL")


@router.callback_query(F.data.startswith("bl_export_uname:"))
async def on_bl_export_uname(callback: CallbackQuery, platform_user: dict | None):
    if not platform_user:
        return
    await _send_bl_csv(callback, platform_user["user_id"],
                       int(callback.data.split(":")[1]),
                       where_extra="AND username IS NOT NULL")

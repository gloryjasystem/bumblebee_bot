...
class RapidApiDelFSM(StatesGroup):
    """Состояния ожидания данных для пайплайна удаления."""
...
async def on_ga_rapidapi_del(call: CallbackQuery, state: FSMContext, platform_user: dict | None):
    """
    ...
    prompt_msg = await call.message.edit_text(
        ...
        "• <b>Пересланное сообщение</b> от нужного пользователя — мгновенно разбаним\n\n"
        "<i>Пользователи будут немедленно разблокированы во всех каналах и группах выборки.</i>",
        parse_mode="HTML",
...
@router.callback_query(F.data.startswith("bs_bl_del:"))
async def on_bl_rapidapi_del(call: CallbackQuery, state: FSMContext, platform_user: dict | None):
    """
    Вход с уровня локального владельца.
    Разбанивает только в чатах указанного child_bot_id.
    """
    if not platform_user:
        return
...
@router.message(RapidApiDelFSM.waiting_for_input, F.forward_origin | F.forward_from)
async def on_rapidapi_del_forward_input(
    ...
    if isinstance(origin, MessageOriginUser):
        ...
        logger.info("[BL DEL] Forward from user=%d, username=%s",
                    user_id, origin.sender_user.username)
    elif msg.forward_from:
        user_id = msg.forward_from.id
        logger.info("[BL DEL] Forward (legacy) from user=%d", user_id)
...
@router.message(RapidApiDelFSM.waiting_for_input, F.text, ~F.forward_origin, ~F.forward_from)
async def on_rapidapi_del_text_input(
    msg: Message, state: FSMContext, bot: Bot, platform_user: dict | None
):
...
@router.message(RapidApiDelFSM.waiting_for_input, F.document)
async def on_rapidapi_del_file_input(
    msg: Message, state: FSMContext, bot: Bot, platform_user: dict | None
):
...
async def _kick_off_pipeline(
    ...
    await state.set_state(RapidApiDelFSM.processing)
    ...
    # Удаляем сообщение с самим вводом пользователя (если это текст или документ)
    try:
        await msg.delete()
    except Exception:
        pass
    ...
    status_msg = await msg.answer(
        f"⏳ <b>Запущено разблокирование {total} записей</b>\n"
    ...
    logger.info(
        "[BL DEL] Pipeline started: owner=%d total=%d (u=%d id=%d) child_bot=%s",

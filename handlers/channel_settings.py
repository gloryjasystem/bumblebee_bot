...
async def _show_requests_menu(callback: CallbackQuery, platform_user: dict, chat_id: int):
    ...
    from utils.timing import effective_delay_sec, format_delay
    auto   = ch["autoaccept"]
    delay  = effective_delay_sec(ch)
    if delay > 0:
        ...
        await db.execute(
            ...
            owner_id, chat_id, timedelta(seconds=delay),
        )
...
async def on_req_auto_toggle(callback: CallbackQuery, platform_user: dict | None):
    ...
    if new_auto:
        # Мгновенное автопринятие — обнуляем отложенную задержку (обе колонки)
        await db.execute(
            "UPDATE bot_chats SET autoaccept=$1, autoaccept_delay=0, autoaccept_delay_sec=0 WHERE owner_id=$2 AND chat_id=$3::bigint",
...
# ── Меню задержки автопринятия (секунды + своё значение) ──────────
# Работает для двух scope: 'ch' (одна площадка) и 'bs' (все площадки бота).

async def _delay_ctx(scope: str, target_id: int, platform_user: dict):
    """Возвращает (owner_id, current_sec). owner_id=None → нет доступа."""
    from utils.timing import effective_delay_sec
    if scope == "bs":
        owner_id = await resolve_owner_id(platform_user["user_id"], target_id)
        if owner_id is None:
            return None, 0
        ch = await _get_bot_first_chat(owner_id, target_id)
    else:
        owner_id = platform_user["user_id"]
        ch = await db.fetchrow(
            "SELECT autoaccept_delay, autoaccept_delay_sec FROM bot_chats "
            "WHERE owner_id=$1 AND chat_id=$2::bigint",
            owner_id, target_id,
        )
    return owner_id, (effective_delay_sec(ch) if ch else 0)


async def _close_bot_later(b, delay_sec: int):
    """Закрывает сессию ad-hoc бота через delay_sec секунд (fire-and-forget)."""
    try:
...
async def _defer_close_for_welcome(b, owner_id: int, chat_ids: list):
    """Закрывает сессию ad-hoc бота ПОСЛЕ завершения цепочки/авто-удаления.
    При ручном приёме приветствие и шаги цепочки планируются как фоновые задачи
    на этом же инстансе; если закрыть сессию сразу — доп. шаги/автоудаление упадут."""
    if not chat_ids:
        try:
            await b.session.close()
        except Exception:
            pass
        return
    step_wait = await db.fetchval(
        "SELECT COALESCE(MAX(delay_sec + COALESCE(self_delete_sec,0)), 0) "
        "FROM welcome_steps WHERE owner_id=$1 AND chat_id = ANY($2::bigint[])",
        owner_id, chat_ids,
    ) or 0
    timer_wait = await db.fetchval(
        "SELECT COALESCE(MAX(welcome_timer), 0) FROM bot_chats "
        "WHERE owner_id=$1 AND chat_id = ANY($2::bigint[])",
        owner_id, chat_ids,
    ) or 0
    max_wait = max(int(step_wait), int(timer_wait))
    if max_wait > 0:
        asyncio.create_task(_close_bot_later(b, max_wait + 15))
    else:
        try:
            await b.session.close()
        except Exception:
...
async def _save_delay(scope: str, target_id: int, owner_id: int, sec: int):
    """Сохраняет задержку в секундах и выключает мгновенное автопринятие."""
    minutes = (sec + 59) // 60  # приближение для устаревшей колонки autoaccept_delay
    if scope == "bs":
        await db.execute(
            "UPDATE bot_chats SET autoaccept=false, autoaccept_delay=$1, autoaccept_delay_sec=$2 "
            "WHERE child_bot_id=$3 AND owner_id=$4",
            minutes, sec, target_id, owner_id,
        )
    else:
        await db.execute(
            "UPDATE bot_chats SET autoaccept=false, autoaccept_delay=$1, autoaccept_delay_sec=$2 "
            "WHERE owner_id=$3 AND chat_id=$4::bigint",
            minutes, sec, owner_id, target_id,
        )
...
_DELAY_PROMPT_TEXT = (
    ...
    "<i>Голое число считается секундами.</i>"
)
_DELAY_ERROR_TEXT = (
    "⚠️ <b>Не понял значение.</b>\n"
    "Пришлите число, можно с единицей:\n"
    "<code>45</code> · <code>5м</code> · <code>1ч</code> · <code>2д</code>.\n"
    "<i>Например: 30 или 5м.</i>"
)
...
def _delay_menu_kb(scope: str, target_id: int) -> InlineKeyboardMarkup:
    ...
    rows = [
        [InlineKeyboardButton(text=t, callback_data=setp + str(v)) for t, v in row]
...
def _delay_menu_text(current_sec: int, saved: bool = False) -> str:
    ...
    return (
        ...
        "<i>Через сколько принимать заявку автоматически.</i>"
    )


async def _show_delay_menu(callback: CallbackQuery, scope: str, target_id: int,
                           current_sec: int, saved: bool = False, edit: bool = False):
    text = _delay_menu_text(current_sec, saved=saved)
    kb = _delay_menu_kb(scope, target_id)
    if edit:
        try:
...
@router.callback_query(F.data.startswith("req_delay:"))
async def on_req_delay_menu(callback: CallbackQuery, state: FSMContext, platform_user: dict | None):
    if not platform_user:
        return
    _, scope, target_id_s = callback.data.split(":")
    target_id = int(target_id_s)
    owner_id, current = await _delay_ctx(scope, target_id, platform_user)
    if owner_id is None:
        await callback.answer("Нет доступа", show_alert=True)
        return
    await state.clear()
    # Открытие — через navigate (замена экрана «Обработка заявок» на сетку пресетов)
    await _show_delay_menu(callback, scope, target_id, current, edit=False)


@router.callback_query(F.data.startswith("req_delay_set:"))
async def on_req_delay_set(callback: CallbackQuery, state: FSMContext, platform_user: dict | None):
    """Нажат пресет (или «🚫 Выкл») — сохраняем сразу и перерисовываем сетку с галочкой."""
    if not platform_user:
        return
    _, scope, target_id_s, sec_s = callback.data.split(":")
    target_id = int(target_id_s)
    owner_id, _current = await _delay_ctx(scope, target_id, platform_user)
    if owner_id is None:
        await callback.answer("Нет доступа", show_alert=True)
        return
    from utils.timing import format_delay, _MAX_DELAY_SEC
    new_sec = max(0, min(_MAX_DELAY_SEC, int(sec_s)))
    await _save_delay(scope, target_id, owner_id, new_sec)
    await state.clear()
    await callback.answer(f"⏰ {format_delay(new_sec)}")
    await _show_delay_menu(callback, scope, target_id, new_sec, saved=True, edit=True)
...
async def on_req_delay_custom(callback: CallbackQuery, state: FSMContext, platform_user: dict | None):
    ...
    try:
        await callback.message.edit_text(
            _DELAY_PROMPT_TEXT, reply_markup=_delay_prompt_kb(scope, target_id), parse_mode="HTML")
    except Exception:
        pass
    await state.set_state(SettingsFSM.waiting_for_delay)
    await state.update_data(
        delay_scope=scope,
        delay_target_id=target_id,
        delay_owner_id=owner_id,
        delay_prompt_chat_id=callback.message.chat.id,
        delay_prompt_msg_id=callback.message.message_id,
    )
    await callback.answer()


@router.message(SettingsFSM.waiting_for_delay)
async def on_req_delay_input(message: Message, state: FSMContext):
    """Ввод «Своё»: парсит секунды/минуты/часы/дни. Ошибка — правкой того же окна."""
    data = await state.get_data()
    if not data or "delay_target_id" not in data:
        return
    scope = data["delay_scope"]
    target_id = data["delay_target_id"]
    owner_id = data["delay_owner_id"]
    prompt_chat_id = data["delay_prompt_chat_id"]
    prompt_msg_id = data["delay_prompt_msg_id"]

    # Удаляем сообщение пользователя (в личке боту это разрешено) — окно остаётся одно.
    try:
        await message.bot.delete_message(message.chat.id, message.message_id)
    except Exception:
        pass

    from utils.timing import parse_delay_input, format_delay
    new_sec = parse_delay_input(message.text or "")
    if new_sec is None:
        # Ошибка ввода — та же плашка + красная строка, остаёмся в состоянии.
        try:
            await message.bot.edit_message_text(
                _DELAY_ERROR_TEXT, chat_id=prompt_chat_id, message_id=prompt_msg_id,
                reply_markup=_delay_prompt_kb(scope, target_id), parse_mode="HTML")
        except Exception:
            pass
        return
    ...
    await _save_delay(scope, target_id, owner_id, new_sec)
    await state.clear()
    # Успех — возвращаем сетку пресетов в том же окне, с галочкой и новым значением.
    try:
        await message.bot.edit_message_text(
            _delay_menu_text(new_sec, saved=True),
            chat_id=prompt_chat_id, message_id=prompt_msg_id,
            reply_markup=_delay_menu_kb(scope, target_id), parse_mode="HTML")
    except Exception:
        pass
...
async def on_req_confirm(callback: CallbackQuery, bot: Bot, state: FSMContext, platform_user: dict | None):
    ...
    if bot_row:
        # Не закрываем сессию сразу — цепочка/автоудаление ещё могут работать на этом инстансе
        await _defer_close_for_welcome(child_bot, owner_id, list(chat_ids))
...
async def _show_bs_requests(callback: CallbackQuery, platform_user: dict, child_bot_id: int):
    ...
    if delay > 0:
        ...
        await db.execute(
            ...
            owner_id, child_bot_id, timedelta(seconds=delay),
        )
...
async def on_bs_req_auto(callback: CallbackQuery, platform_user: dict | None):
    ...
    if new_val:
        # Мгновенное автопринятие — обнуляем отложенную задержку (обе колонки)
        await db.execute(
            "UPDATE bot_chats SET autoaccept=$1, autoaccept_delay=0, autoaccept_delay_sec=0 WHERE child_bot_id=$2 AND owner_id=$3",
    ...
    for chat_row in chats:
        ...
        for row in pending:
            try:
                ...
                pass
    if bot_row:
        await _defer_close_for_welcome(child_bot_instance, owner_id, [c["chat_id"] for c in chats])
...
async def on_bs_base(callback: CallbackQuery, platform_user: dict | None):
    ...
    await callback.message.edit_text(
        ...
        f"🚫 Заблокировано ботом: {blocked_count:,}\n"
        f"🛡 ЧС: {bl_status}",
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
...
async def _show_bs_bl_manage(callback: CallbackQuery, platform_user: dict,
    ...
    await callback.message.edit_text(
        ...
        reply_markup=InlineKeyboardMarkup(inline_keyboard=[
            ...
            [InlineKeyboardButton(text="➕ Добавить в ЧС",
                                  callback_data=f"bl_rapidapi_add:{child_bot_id}")],
            [InlineKeyboardButton(text="➖ Удалить из ЧС",

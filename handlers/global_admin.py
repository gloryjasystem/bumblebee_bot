...
# ══════════════════════════════════════════════════════════
# ВРЕМЕННЫЙ ХЕНДЛЕР СБОРА ID КАСТОМ-ЭМОДЗИ — удалить после сбора
# Использование: одним сообщением прислать
#     /emojiid  ⏱🔒👋…   (премиум-эмодзи из паков сразу после команды)
# Бот отвечает списком «эмодзи → custom_emoji_id» для словаря замен.
# Ничего в продукте не меняет: только читает entities входящего сообщения.
# ══════════════════════════════════════════════════════════
@router.message(Command("emojiid"))
async def cmd_emojiid(message: Message):
    from config import settings
    allowed = {settings.owner_telegram_id, settings.co_owner_telegram_id}
...
async def _build_network_text(owner_id: int, conn) -> str:
    ...
    return (
        ...
        f"├ Глобальный ЧС: <b>{fmt(total_global_bl)}</b> записей\n"
        f"└ Локальные ЧС ботов: <b>{fmt(total_local_bl)}</b> записей\n\n"
        "⚡ <b>ТРАФИК И АУДИТОРИЯ</b>\n"
        f"├ Пользователей в ботах: <b>{fmt(total_audience)}</b> чел.\n"
        f"├ Новых за 24 часа: <b>+{fmt(new_users_24h)}</b>\n"

...
async def _show_admin_panel(message_or_cb, role: str, owner_id: int):
    ...
    if isinstance(message_or_cb, Message):
        ...
        await safe_edit_text(message_or_cb, header, reply_markup=markup, parse_mode="HTML")
...
async def on_ga_team(callback: CallbackQuery):
    ...
    await safe_edit_text(callback, text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
    await callback.answer()
...
async def on_ga_revenue(event: Message | CallbackQuery):
    ...
    if isinstance(event, Message):
        ...
        await safe_edit_text(event, text, parse_mode="HTML", reply_markup=InlineKeyboardMarkup(inline_keyboard=kb))
        await event.answer()
...
async def on_ga_bl_export_csv(callback: CallbackQuery):
    ...
    msg = await safe_edit_text(callback, "⏳ Генерирую CSV чёрного списка из выбранных ботов...")
    import asyncio
...
async def _show_ga_users(message_or_cb, admin_id: int, owner_id: int):
    ...
    if isinstance(message_or_cb, Message):
        ...
        await safe_edit_text(message_or_cb, text, reply_markup=markup, parse_mode="HTML")
...
async def on_ga_export_users(callback: CallbackQuery):
    ...
    msg = await safe_edit_text(callback, "⏳ Идёт сбор глобальной базы и генерация CSV. Пожалуйста, подождите...")
    import asyncio

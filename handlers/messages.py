...
async def _show_captcha(callback: CallbackQuery, chat_id: int, owner_id: int):
    ...
    if not has_welcome:
        ...
        greet_desc     = "приветствие и цепочка не заданы — отправлять нечего."
    elif greet_m == 1:   # до капчи
        greet_btn_text = "📨 Сообщения: до капчи"
        greet_desc     = "сначала приветствие и цепочка, потом капча."
    else:                # после капчи (гейт, дефолт)
        greet_btn_text = "📨 Сообщения: после капчи"
        greet_desc     = "сначала капча, потом приветствие и цепочка."

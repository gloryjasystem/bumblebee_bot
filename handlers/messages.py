...
async def _show_ch_messages(callback: CallbackQuery, chat_id: int, owner_id: int):
    ...
    await navigate(
        ...
        "​🗑 <b>Удаление сообщений</b> — задаёт срок жизни автоответам бота: через выбранное время они сами удаляются, чтобы диалог не копил старые ответы. Приветствие и рассылки остаются.\n\n"  # ​ = маркер: эта 🗑 остаётся обычной (не премиум)

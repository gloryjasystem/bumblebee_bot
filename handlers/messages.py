...
# ВРЕМЕННО: «1» — короткий вариант для быстрой сверки «срока жизни»; убрать после теста.
_DELETE_CYCLE = [0, 1, 30, 60, 180, 360, 720, 1440, 2160, 2880]
...
async def _show_ch_messages(callback: CallbackQuery, chat_id: int, owner_id: int):
    ...
    farewell_label = f"🤚 Прощание: {'вкл' if (has_farewell and farewell_on) else 'выкл'}"
...
async def _show_captcha(callback: CallbackQuery, chat_id: int, owner_id: int, back_cb: str | None = None):
    ...
    if not has_welcome:
        greet_btn_text = "✉️ Запуск цепочки: — (не задана)"
        greet_desc     = "приветствие и цепочка не заданы — запускать нечего."
    elif greet_m == 1:   # до капчи
        greet_btn_text = "📨 Запуск цепочки: до капчи"
        greet_desc     = "цепочка идёт сразу, капча — в конце."
    else:                # после капчи (гейт, дефолт)
        greet_btn_text = "📨 Запуск цепочки: после капчи"
        greet_desc     = "цепочка стартует после прохождения капчи."

    # ​ (ZWSP) перед 📨 — оставить конверт ОБЫЧНЫМ (не премиум, как в кнопке), см. middlewares/custom_emoji.py
    info = (
        "🔒 <b>Капча</b>\n\n"
        "⚪ <b>Простая:</b> пользователю достаточно нажать на любую кнопку.\n\n"
        "🔵 <b>Рандомная:</b> пользователю необходимо нажать на верную кнопку.\n\n"
        "<blockquote>"
        f"​📨 <b>Запуск цепочки:</b> {greet_desc}\n\n"

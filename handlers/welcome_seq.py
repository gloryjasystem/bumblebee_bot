...
def _plain(text: str) -> str:
    """HTML-текст шага → чистый видимый текст (снимает теги форматирования и entity).

    Текст шага хранится как HTML (форматирование). В коротком превью его НЕЛЬЗЯ
    обрезать «как есть»: обрезка может порвать тег (напр. открытый <b> без </b>),
    и Telegram уронит ВЕСЬ экран цепочки (Bad Request: can't parse entities).
    Поэтому в превью показываем plain-текст; обрезка plain-текста безопасна, а при
    вставке в HTML-пузырь превью заново экранируется (_html.escape)."""
    return _html.unescape(_HTML_TAG_RE.sub("", text or ""))
...
def _step_preview(st, n: int = 22):
    ...
    txt = _plain(st["text"]).strip().replace("\n", " ")

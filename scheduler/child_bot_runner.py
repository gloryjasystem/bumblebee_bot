...
# ── Дедуп заявок между ботами ──────────────────────────────────
# Один канал может быть подключён к нескольким ботам-админам, и Telegram шлёт заявку
# на вступление КАЖДОМУ из них. Без дедупа оба бота обрабатывают одну заявку параллельно:
# один шлёт капчу, другой в это же время принимает — рассинхрон («капча пришла, а я уже в
# канале»). Поэтому первую по времени обработку по паре (chat_id, user_id) «занимаем», а
# повторную от другого бота в пределах окна — пропускаем. Один процесс на все дочерние боты,
# поэтому обычного dict в памяти достаточно.
_recent_join_req: dict[tuple[int, int], float] = {}
# Окно короткое: гонка двух ботов на одну заявку — миллисекунды, а окно не должно мешать
# быстрым повторным заходам того же пользователя (вступил/вышел/снова при тестах).
_JOIN_REQ_DEDUP_SEC = 5


def _join_request_claimed(chat_id: int, user_id: int) -> bool:
    """True, если заявку этой пары уже взял в обработку другой бот в пределах окна.
    Побочный эффект: помечает пару как «занята сейчас» и чистит устаревшие записи."""
    now = time.monotonic()
    if len(_recent_join_req) > 512:
        for k, ts in list(_recent_join_req.items()):
            if now - ts > _JOIN_REQ_DEDUP_SEC:
                _recent_join_req.pop(k, None)
    key = (chat_id, user_id)
    last = _recent_join_req.get(key)
    if last is not None and now - last < _JOIN_REQ_DEDUP_SEC:
        return True
    _recent_join_req[key] = now
    return False
            ...
            if chats:
                ...
                if _canon:
                    ...
                    if welcome_row.get("welcome_text") or welcome_row.get("welcome_media"):
                        ...
                        # from_join_request=True: /start — законный прямой запрос, поэтому приветствие
                        # шлём даже для режима «Сразу (по заявке)» (welcome_delay_sec<0). Иначе гард в
                        # _send_welcome заглушил бы /start у таких каналов (лента не приходила). Дубля
                        # нет: /start разовый + защита _welcome_already_sent (180с).
...
async def _handle_join_request(bot: Bot, child_bot_id: int, event: ChatJoinRequest):
    ...
    # Дедуп между ботами: одну заявку обрабатывает только ПЕРВЫЙ получивший её бот.
    # На общем канале Telegram шлёт заявку всем ботам-админам; без этого оба обработают её
    # параллельно — один пришлёт капчу, другой в это же время примет (капча оказывается «мёртвой»).
    if _join_request_claimed(chat_id, user.id):
        logger.info(
            f"[JOIN REQ] уже обрабатывается другим ботом канала — пропуск user={user.id} chat={chat_id}"
        )
        return
